...
lzma_index_padding_size(const lzma_index *i)
{
	return (LZMA_VLI_C(4) - index_size_unpadded(
								i->record_count, i->index_list_size)) &
		   3;
}
					  ...
					  lzma_vli unpadded_size, lzma_vli uncompressed_size)
{
	...
	if (index_size(i->record_count + 1,
				   i->index_list_size + index_list_size_add) > LZMA_BACKWARD_SIZE_MAX)
		return LZMA_DATA_ERROR;
				   ...
				   const lzma_allocator *allocator)
{
	...
	// to be done before modifying dest or src.
	{
		...
		if (g != NULL && g->last + 1 < g->allocated)
		{
			...
			index_group *newg = lzma_alloc(sizeof(index_group) + (g->last + 1) * sizeof(index_record),
										   allocator);
			if (newg == NULL)
				return LZMA_MEM_ERROR;
...
index_dup_stream(const index_stream *src, const lzma_allocator *allocator)
{
	...
	index_group *destg = lzma_alloc(sizeof(index_group) + src->record_count * sizeof(index_record),
									allocator);
	if (destg == NULL)
	{
...
iter_set_info(lzma_index_iter *iter)
{
	...
	if (group != NULL)
	{
		...
		iter->block.compressed_stream_offset = record == 0 ? group->node.compressed_base
														   : vli_ceil4(group->records[record - 1].unpadded_sum);
		iter->block.uncompressed_stream_offset = record == 0 ? group->node.uncompressed_base
